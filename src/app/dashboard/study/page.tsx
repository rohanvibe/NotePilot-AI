...
import { BookOpen, Sparkles, CheckCircle, XCircle, RefreshCw, GraduationCap, ChevronRight, Brain } from "lucide-react";
        ...
        <div className="max-w-4xl mx-auto py-12 px-6 space-y-12">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="space-y-2">
                    ...
                    <p className="text-slate-400 font-medium">Test your knowledge with AI-driven challenges.</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="p-8 rounded-[40px] bg-white/5 border border-white/10 space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-slate-500">Generate a quiz from all your knowledge sources across all topics.</p>
                        </div>
                        <button
                    ...
                    <div className="p-8 rounded-[40px] bg-slate-900/40 border border-white/5 space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-slate-500">Master specific subjects by selecting a topic below.</p>
                        </div>
            ...
            {isGenerating && (
                <div className="py-20 text-center space-y-6">
                    ...
                    <p className="text-xl font-bold animate-pulse text-emerald-400">Synthesizing practice questions...</p>
                </div>
            )}
                ...
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {quiz[currentQuestion].options.map((option: string, idx: number) => {
                            ...
                            let styles = "bg-white/5 border-white/10 text-slate-300 hover:bg-white/10";
                            if (selectedAnswer !== null) {
                                if (isCorrect) styles = "bg-emerald-500/20 border-emerald-500/50 text-emerald-400";
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {selectedAnswer !== null && isCorrect && <CheckCircle className="w-6 h-6 shrink-0" />}
                                    {isSelected && !isCorrect && <XCircle className="w-6 h-6 shrink-0" />}
                                    {selectedAnswer === null && <ChevronRight className="w-5 h-5 opacity-0 group-hover:opacity-100 transition-opacity" />}
                                </button>
                            );
                        })}
                    </div>
            ...
            {isFinished && (
                <div className="text-center py-20 space-y-10 bg-white/5 border border-white/5 rounded-[64px] animate-in zoom-in duration-700">
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-slate-400 text-xl font-medium">You dominated {score} / {quiz.length} concepts</p>
                    </div>

                    <div className="max-w-xs mx-auto pt-6 flex flex-col gap-4">
                        <button
                            onClick={() => { setQuiz([]); setIsFinished(false); }}
